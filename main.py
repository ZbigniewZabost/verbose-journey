...
def login(driver, url, email, password):
    print("Signing into {} with email {}".format(BASE_URL, EMIAL))
    driver.get(url)
...
def scrap_for_images_url(driver):
    ...
    for e in entries:
        ...
        photos_elements = driver.find_elements(By.XPATH, "//div[contains(@class, 'carousel-item')]/img[@loading='lazy']")
        for p in photos_elements:
            images_urls.append(p.get_attribute("src"))
        attachments = driver.find_elements(By.XPATH, "//table/tbody/tr/td/a[contains(@class, 'btn-light') and contains(@class, 'btn')]")
...
def scrap_site():
    ...
    print("\tFound {} images in galleries and {} files in attachments".format(GALLERY_IMAGES_COUNT, ATTACHMENTS_FILES_COUNT))
